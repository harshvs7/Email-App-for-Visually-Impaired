        ...
        tts = TextToSpeech(applicationContext, TextToSpeech.OnInitListener {
            if(it == TextToSpeech.SUCCESS){
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    tts.speak("Whom do you want to sent the mail, Speak by clicking anywhere at the bottom right of the screen",TextToSpeech.QUEUE_FLUSH,null,null);
                } else {
                    tts.speak("Whom do you want to sent the mail, Speak by clicking anywhere at the bottom right of the screen", TextToSpeech.QUEUE_FLUSH, null);
                }
            }
        })
...
//            val temp = result?.get(0).toString()
            if(count==0){
                mailTo.setText(value?.get(0).toString().replace("\\s".toRegex(), "").toLowerCase())
            }
            else if(count==1){
